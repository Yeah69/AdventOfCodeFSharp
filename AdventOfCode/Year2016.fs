...
module Day1 =
    ...
    let getInstructions (input:string) =
        let textInstructions = input.Split([| ", " |], StringSplitOptions.None)
        textInstructions 
...
module Day2 =
    ...
    let calculate lines step =
        ...
        |> asFirst ('5', "")
        ||> Seq.fold (fun (lastNumber, currentSum) (line: string, _) ->
            let nextNumber =
                line
...
module Day11 =
    ...
    let getFloor aComponent = match aComponent with | Microchip (_, floor) | Generator (_, floor) -> floor
    
    let getLabel aComponent = match aComponent with | Microchip (label, _) | Generator (label, _) -> label
    
    let compWithFloor floor aComponent = 
        match aComponent with 
        | Microchip (label, _) -> Microchip (label, floor)
        | Generator (label, _) -> Generator (label, floor)
        
    let isMicrochip aComponent = 
        match aComponent with 
        | Microchip _ -> true
        | Generator _ -> false
        
    let isGenerator aComponent = 
        match aComponent with 
        | Microchip _ -> false
        | Generator _ -> true
        ...
        |> Seq.choose (fun line ->
            ...
            | Regex "The (first|second|third|fourth) floor contains(.*)\." (textFloor::textList::[]) ->
                let floor =
                    ...
                    | "third" -> F3
                    | _ -> F4
                Some (textList.Split([| " a " |], StringSplitOptions.None)
...
module Day12 =    
    ...
    let runProgram = runProgram' (fun prog -> prog.Instructions[prog.InstructionPointer] prog)
...
module Day21 =
    ...
    let reverse start finish (currText:string) =
        ...
        let reversed = System.String(subString |> Seq.rev |> Seq.toArray)

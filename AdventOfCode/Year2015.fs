...
module Day4 =
    let go() =
        ...
        let sequence =
            ...
            |> Seq.map (fun (i, message) -> i, md5 message)
            ...
            |> Seq.unfold (fun (map, assigneeToOperation) ->
                if assigneeToOperation |> Map.isEmpty then
                    ...
                    let map, assigneeToOperation =
                        ...
                        |> asFirst (map, assigneeToOperation)
                        ||> Seq.fold (fun (map, assigneeToOperation) (assignee, operation) -> 
                            let map = (map, operation) ||> doOperation
                            let assigneeToOperation = (assignee, assigneeToOperation) ||> Map.remove
                            (map, assigneeToOperation))
...
module Day13 =
    ...
    let go() =
        ...
        let result1, result2 =
            ([], setOfNames) 
            ||> permutations
            |> asFirst (Integer.MinValue, Integer.MinValue)
